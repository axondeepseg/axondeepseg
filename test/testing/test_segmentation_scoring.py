...
class TestCore(object):
    def setup(self):
        ...
        self.img = imread(os.path.join(self.folderPath,'image.png'), flatten=True)
        self.groundtruth = imread(os.path.join(self.folderPath,'mask.png'), flatten=True)
        self.prediction = imread(os.path.join(self.folderPath,'AxonDeepSeg_seg-axonmyelin.png'), flatten=True)
    ...
    def test_score_analysis_returns_nonzero_outputs_for_differing_overlapping_masks(self):
        ...
        # Note that if imageio.imread(imagefile) was used instead of scipy's imread with flatten = true, the scores would be 0 (from experience....
    ...
    def test_pwdice_returns_nonzero_value_for_differing_overlapping_masks(self):
        ...
        predMyelin = np.logical_and(self.prediction >= 50, self.prediction <= 200)
