...
class TestCore(object):
    ...
    # **NOTE** Because most data augmentation functions chose the parameters randomly 
    # within bounds set by the arguments, it's impossible to target test cases for known
    # shifts, rotations, etc. Thus, only broad tests are defined (output dim = input dim,
    # output patch different or same as input patch)
    ...
    def test_large_max_rescaling_factor_returns_different_image(self):
        ...
        assert augmentedPatch[0].shape == patch[0].shape
        assert augmentedPatch[1].shape == patch[1].shape
        assert np.not_equal(augmentedPatch[0], patch[0]).any(), """Note: due to  therandomized rescaling factor choice,
    ...
    def test_gaussian_blur_returns_image_of_same_size(self):
        # Because the sigma blur size is also random, it's very difficult to test for a known case (different/same image), so this low-level test only covers the same image size assertions.
        patch = [self.testImage, self.mask]
