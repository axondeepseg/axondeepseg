...
def download_model(destination = None):
    ...
    if (
        not download_data(url_TEM_model) and not download_data(url_SEM_model) and not download_data(url_model_seg_pns_bf) and not download_data(url_model_seg_rat_axon_myelin_sem)
    ) == 1:
