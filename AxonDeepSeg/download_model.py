...
def download_model(destination = None):
    ...
    if destination is None:
        ...
        om_destination = Path("AxonDeepSeg/models/model_seg_rat_axon-myelin_bf")
    else:
        sem_destination = destination / "model_seg_rat_axon-myelin_sem"
        tem_destination = destination / "model_seg_mouse_axon-myelin_tem"
        om_destination = destination / "model_seg_rat_axon-myelin_bf"
    ...
    url_om_destination = "https://github.com/axondeepseg/default-OM-model/archive/refs/tags/" + om_release_version + ".zip"

    files_before = list(Path.cwd().iterdir())

    if (
        not download_data(url_sem_destination) and not download_data(url_tem_destination) and not download_data(url_om_destination) 
    ) == 1:
    ...
    folder_name_OM_model = Path("default-OM-model-" + om_release_version)
    ...
    if om_destination.exists():
       print('OM model folder already existed - deleting old one')
       shutil.rmtree(str(om_destination))
    ...
    shutil.move(folder_name_OM_model.joinpath("model_seg_rat_axon-myelin_bf"), str(om_destination))
