...
def train_model(path_trainingset, path_model, config, path_model_init = None, save_trainable = True, verbose = 1):
    ...
    def Uconv_net(x, weights, biases, dropout, image_size = image_size):
        ...
        data_temp  = tf.nn.conv2d(input=data_temp, filter=weights['dense'], strides=[1, 1, 1, 1], padding="SAME")
        data_temp  = tf.nn.bias_add(data_temp , biases['dense_b'])
        data_temp  = tf.nn.relu(data_temp )
        
        finalconv = tf.nn.conv2d(data_temp, weights['finalconv'], strides=[1, 1, 1, 1], padding='SAME')
    ...
    weights['dense'] = tf.Variable(tf.random_normal([image_size,image_size,num_features_in,num_features_in]), name='denseconv-%s'%i)
    biases['dense_b']= tf.Variable(tf.random_normal([num_features_in]), name='bdenseconv-%s'%i)

    # num_features_in = num_features_in*image_size*image_size
