...
def apply_convnet(path_my_data, path_model,config,thresh_indices = [0,0.5]):
    """
...
def axon_segmentation(path_my_data, path_model, config, thresh_indices = [0,0.5]):
    """
    :param path_my_data: folder of the image to segment. Must contain image.jpg
    :param path_model: folder of the model of segmentation. Must contain model.ckpt
    :return: no return
    ...
    # ------ Apply ConvNets ------- #
    prediction = apply_convnet(path_my_data, path_model, config,thresh_indices = thresh_indices)
